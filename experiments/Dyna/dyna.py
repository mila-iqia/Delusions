...
class MINIGRID_OBS_ONESTEP_MODEL(torch.nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        super(MINIGRID_OBS_ONESTEP_MODEL, self).__init__(**kwargs)
    ...
    def logits_from_action(self, action, context):
        ...
        samples_decompressed = self.decompressor(ebd_action)
    ...
    def imagine_transition_from_obs(self, obses):
        size_batch = obses.shape[0]
        action_sampled = torch.randint(0, self.num_actions, (size_batch, 1), device=obses.device)
...
class DQN_DYNA(DQN_DYNA_BASE):
    ...
    def update(self, batch_processed=None, writer=None):
        ...
        with torch.no_grad():
            ...
            action_imagined, reward_imagined, obses_imagined, omegas_imagined = self.model.imagine_transition_from_obs(batch_obs_curr)
        ...
        if self.optimizer_evaluator is not None:
            ...
            if self.prioritized_replay:
                ...
                error_evaluator_weighted = (loss_discount * weights.squeeze().detach()).mean()
            else:
            ...
            if debug:
                with torch.no_grad():
                    grads = [param.grad.detach().flatten() for param in self.parameters_evaluator]
        ...
        if self.optimizer_evaluator is not None:
            with torch.no_grad():
                ...
                mask_rejected = dist_distance2imagined[:, 0] < 0.25
        ...
        if self.prioritized_replay:
            assert weights is not None
            error_dyna_weighted = loss_model.mean() + ((loss_Q + loss_Q_dyna) * weights.detach()).mean()
        else:
        ...
        if debug:
            with torch.no_grad():
                grads = [param.grad.detach().flatten() for param in self.parameters_dyna]
