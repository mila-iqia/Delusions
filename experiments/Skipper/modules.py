...
def bottlenecked_multi_head_attention_forward(
    ...
    if attn_output_weights is None:
        ...
        if size_bottleneck == 1:
            attn_weights_topk, indices_topk = torch.topk(softmax(attn_output_logits, dim=-1), dim=-1, k=1, sorted=False)
            attn_output_weights = torch.zeros_like(attn_output_logits).scatter_(-1, indices_topk, attn_weights_topk)
        elif size_bottleneck < attn_output_logits.shape[-1]:
