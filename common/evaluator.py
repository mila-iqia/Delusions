...
class HistogramConverter(torch.nn.Module):
    ...
    def to_histogram(self, value: torch.Tensor):
        value = value.clamp(self.value_min, self.value_max)
        value_normalized = (value - self.value_min) * self.const_norm
...
class TargetEvaluator(torch.nn.Module):
    def __init__(
        ...
        if self.create_optimizer:
            ...
            self.optimizer = eval("torch.optim.%s" % type_optimizer)(self.params_optim, lr=lr, eps=eps)
        else:
            self.optimizer = None
