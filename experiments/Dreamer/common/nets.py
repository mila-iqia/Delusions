...
class RSSM(common.Module):
    ...
    def img_step(self, prev_state, prev_action, sample=True, prob_rand=0.0):
        ...
        #
        if prob_rand > 0:
            logits = stats.pop('logit')
            probs =  torch.full_like(logits, prob_rand) + (1.0 - prob_rand) * logits.softmax(-1) # NOTE: to exaggerate the hallucination problem
            stats["logit"] = torch.log(probs)
        dist = self.get_dist(stats)
        stoch = dist.sample() if sample else dist.mode
