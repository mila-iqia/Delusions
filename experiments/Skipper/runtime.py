...
def generate_exptag(args, additional=""):
    ...
    if "minigrid" in args.game.lower() or "randdistshift" in args.game.lower() or "swordshieldmonster" in args.game.lower():
        ...
        if "leap" in args.method.lower():  # not distributional
        ...
        if args.method == "DQN_Skipper":
            ...
            if args.nonsingleton:
                args.comments += "_nonsingleton"
        if args.clip_reward:
        ...
        elif "dyna" in args.method.lower():
            if args.dyna_reject:
                args.comments += f"_reject"
        else:
...
def get_new_env_RDS(args, size=8, lava_density_range=[0.3, 0.4], min_num_route=1, uniform_init=False, gamma=0.99, stochasticity=0.0, singleton=True):
    env = gym.make(
...
def get_new_env_SSM(args, size=8, lava_density_range=[0.3, 0.4], uniform_init=False, gamma=0.99, stochasticity=0.0, singleton=True):
    env = gym.make(
...
def config_parser(mp=True):
    ...
    parser.add_argument("--method", type=str, default="DQN_Skipper", help="type of agent")  # DQN_Skipper
    ...
    # for Dyna only
    parser.add_argument("--dyna_reject", type=int, default=1, help="1 to enable rejection")
    # for non-singleton targets (with Skipper)
    parser.add_argument("--nonsingleton", type=int, default=0, help="")
