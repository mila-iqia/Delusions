...
from utils import init_weights, HistogramConverter, cyclical_schedule #, reparameterize, clamp_with_grad  # , unique_rows
from modules import ResidualBlock, TopKMultiheadAttention  # , NoisyLinear
...
class Decoder_MiniGrid_RDS(torch.nn.Module):
    ...
    def forward(self, layout, mask_agent):
        ...
        # NOTE: in RDS, the lavas are not by default printed, only the agent changes color when doing shit
...
class Decoder_MiniGrid_SSM(torch.nn.Module):
    ...
    def forward(self, layout, mask_agent_ijx):
        ...
        # we can put swords and shields according to x and then in the end paint whatever object on the two spots the right color
        # this automatically fixes the x does not match obs problem, where the agent is on top of the sword or shield with wrong x, e.g. x = 0
        assert obs.dtype == torch.uint8
        return obs
...
class Embedder_MiniGrid_BOW(torch.nn.Module):  # adapted from BabyAI 1.1
    def __init__(self, max_value=32, dim_embed=8, channels_obs=2, height=8, width=8, ebd_pos=False):
        ...
        self.embedding = torch.nn.Embedding(self.channels_obs * self.max_value, dim_embed)  # NOTE: +2 for X and Y
...
class Predictor_MiniGrid(torch.nn.Module):
    ...
    def __init__(
        ...
        for idx_layer in range(depth):
            ...
            # self.layers.append(NoisyLinear(len_in, len_out))
